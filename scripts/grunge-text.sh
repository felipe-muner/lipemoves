#!/usr/bin/env bash
# Distress an RGBA text PNG into the "grunge stamp" look: a bold fill with a thin
# dark keyline, rough eroded edges, fine diagonal scratches inside the letters,
# and a soft drop shadow. Shared by cover.sh and label-video.sh when the studio's
# "Grunge" toggle is on.
#
...
OUTLINE_PX=7                 # thin dark keyline width (at the reference render size)
...
# --- Edge roughening: chew the outer boundary but keep the interior solid. ---
...
"$MAGICK" -size "${W}x${H}" xc: -seed 11 +noise Random -colorspace Gray \
  -blur 0x0.6 -threshold 40% "$TMP/edge.png"
# Ragged fringe = glyph ∩ noise; solid core = glyph eroded a touch.
"$MAGICK" "$TMP/ga.png" "$TMP/edge.png" -compose Multiply -composite "$TMP/roughb.png"
"$MAGICK" "$TMP/ga.png" -morphology Erode "Disk:3" "$TMP/core.png"
# Fill alpha = solid core OR ragged fringe (opaque inside, ragged edges).
"$MAGICK" "$TMP/core.png" "$TMP/roughb.png" -compose Lighten -composite "$TMP/fillA.png"

# --- Diagonal scratch hatch (dark marks inside the letters). ---
...
"$MAGICK" -size "${W}x${H}" xc: -seed 29 +noise Random -colorspace Gray \
  -threshold 50% -motion-blur 0x22+52 -threshold 55% "$TMP/hatch.png"
"$MAGICK" "$TMP/fillA.png" "$TMP/hatch.png" -compose Multiply -composite "$TMP/scrA.png"

# --- Thin dark keyline: dilate the fill alpha and sit a black ring underneath. ---
...
"$MAGICK" -size "${W}x${H}" xc:black \( "$TMP/scrA.png" \) \
...
"$MAGICK" "$TMP/stamp.png" \
  \( +clone -background black -shadow "85x5+0+6" \) +swap \
