#!/usr/bin/env bash
# Cover/thumbnail maker (gradient + WHITE outline variant): take an image (a
# video frame) + a phrase, and burn full-width text filled with ONE soft green
# gradient that flows diagonally across the entire block (top-left first letter
# -> bottom-right last letter), plus a thick WHITE outline. Viral-fitness style.
#
# Usage:
#   ./scripts/cover-gradient-write.sh <image> "YOUR PHRASE" [out.jpg] [top|bottom|center]
#
# Notes:
